...
(function commandOriginatedMessage() {
	...
	const messageHandler = sinon.spy(function () {
	...
	test('before all commandOriginatedMessage', function (t) {
		...
		receiveChannel.respond({role: 'test', cmd: 'command:message'}, messageHandler);

		readTransport.on('message:handled', function () {
			t.end();
		});

		// Send the command.
		sendChannel.request({role: 'test', cmd: 'command:message'}, paylaod);
	});

...
var Message = require('./inbound-message');

// spec.sqs
// spec.queueUrl
// spec.debug
function LongPoll(spec) {
...
utils.extend(LongPoll.prototype, {
	start: function () {
		if (!this.isRunning) {
			this.isRunning = true;
			this.runLoop();
				...
				.then(function (messages) {
					if (self.isRunning) {
						messages.forEach(function (rawMessage, i) {
							var message = Message.create(rawMessage);
							setTimeout(function () {
...
// spec.queueUrl
// spec.debug
LongPoll.create = function (spec) {
	return new LongPoll(spec);
};
